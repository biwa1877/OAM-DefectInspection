% This script makes all the images needed for the flowchart figure, which
% contains, so far,:
...
%     Fesw2_d2 = fftshift(fft2(ifftshift(ESW2_d2)));
    ...
    figure(23);
    subplot(221);imagesc(abs(Fesw1_d1));axis image off;colormap hot;
    subplot(223);imagesc(abs(Fesw2_d1));axis image off;
    ...
    figure(24);
    subplot(221);imagesc(abs(diff).^0.5 .* sign(diff));axis image off;colormap hot;
    subplot(223);imagesc(abs(diff_crop).^0.5 .* sign(diff_crop));axis image off;
    ...
    figure(25);
    subplot(221);imagesc(abs(IFFTdiff));axis image off;colormap hot;
    subplot(222);imagesc(angle(IFFTdiff));axis image off;
    subplot(223);imagesc(abs(IFFTdiff_crop));axis image off;
    subplot(224);imagesc(angle(IFFTdiff_crop));axis image off;
    
    posi = [1011 95 890 871];
    set(gcf, 'Position', posi);
