...
% 
% Version control:
% v01: does tight subplot using the "tight_subplot" function
...
%     subplot(nrow, ncol, 2 + ncol);
    ...
    I1_d1 = I1_d1 + randn(size(I1_d1)) * max(abs(I1_d1(:))) * wgnLevel(i);
    I2_d1 = I2_d1 + randn(size(I2_d1)) * max(abs(I2_d1(:))) * wgnLevel(i);
...
%     diff = diff + randn(size(diff)) * max(abs(diff(:))) * wgnLevel(i);
    ...
    cropRange = 99:157;
    mask(cropRange,cropRange) = 1;
%     diff_crop = diff .* mask;
    diff_crop = diff(cropRange,cropRange);
